...
function validateLocationInput(input) {
    if(input.value.trim() === '') {
        input.setCustomValidity("Location search cannot be blank.");
        return false;
    } else {
...
function locationInputForm() {
    ...
    locationInputSection.innerHTML = `
        <form class="location-input-form" novalidate>
            <input type="text" placeholder="Search..." class="input-form__input-field" required>
    ...
    inputField.addEventListener("input", () => {
        validateLocationInput(inputField);
    });
    ...
    locationInputForm.addEventListener("submit", async (e) => {
        const isValid = validateLocationInput(inputField);
